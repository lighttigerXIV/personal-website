...
import { getProjectsWithStars, Project, projects as PROJECTS } from "@pages/projects/vm"
import Icon from "@/components/icon.vue";
import { openLink, getImageUrl } from "@/main-viewmodel";

const filters = ref(["All", "Android", "Linux", "Windows", "Web"]);
const currentFilter = ref("All");
const projects = ref<Project[]>(PROJECTS);
const currentProjects = ref<Project[]>(PROJECTS)

onMounted(async () => {
    document.title = "LighttigerXIV - Projects";

    getProjectsWithStars((result) => {
        projects.value = result;
        filterProjects(currentFilter.value);
    });
});
                    ...
                    <div v-for="project in currentProjects" class="card grid grid-cols-2 ">
                        ...
                        <div class="flex flex-col ml-[16px]">
                            ...
                            <div class="flex flex-grow flex-col justify-center items-center">
                                ...
                                <div class="flex items-center pt-2 pb-2 pl-4 pr-4 bg-mantle rounded-full">
                                    <div>{{ project.stars }}</div>
                                    <Icon name="star" class="h-4 w-4 ml-2" />
                                </div>

                                <div class="h-[2px] bg-text w-[40px] mt-3 mb-3 rounded-full"></div>
